...
namespace frost::sync
{
	...
	i32 sync_object::acquire_one(const std::vector<sync_object>& synchronization_objects)
	{
		return api::acquire_one(
			reinterpret_cast<const pimpl_crtp<sync_object>*>(synchronization_objects.data()),
	...
	i32 sync_object::api::acquire_one(const pimpl_crtp<sync_object>* synchronization_objects, i32 count)
	{
		if (count > MAXIMUM_WAIT_OBJECTS)
			return false;

		auto* data = const_cast<pimpl_crtp<sync_object>*>(synchronization_objects);
	...
	bool sync_object::api::acquire_all(const pimpl_crtp<sync_object>* synchronization_objects, i32 count)
	{
		if (count > MAXIMUM_WAIT_OBJECTS)
			return false;
	...
	i32 sync_object::api::try_acquire_one(const pimpl_crtp<sync_object>* synchronization_objects, i32 count)
	{
		if (count > MAXIMUM_WAIT_OBJECTS)
			return false;
	...
	bool sync_object::api::try_acquire_all(const pimpl_crtp<sync_object>* synchronization_objects, i32 count)
	{
		if (count > MAXIMUM_WAIT_OBJECTS)
			return false;
