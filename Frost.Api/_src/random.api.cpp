...
u64 frost::api::random::generate(u64* p_seed)
{
	u64 ret = *p_seed;
	ret ^= ret << x;
	ret ^= ret >> y;
...
u64 frost::api::random::generate_range(u64* p_seed, u64 min, u64 range)
{
	u64 ret = *p_seed;
	ret ^= ret << x;
	ret ^= ret >> y;
...
f64 frost::api::random::generate_range_double(u64* p_seed, f64 min, f64 range)
{
	u64 ret = *p_seed;
	ret ^= ret << x;
	ret ^= ret >> y;
